...
export default function Dashboard() {
    ...
    const [mode, setMode] = useState("weekly"); // default
    ...
    useEffect(() => {
        const fetchChartData = async () => {
            try {
                const res = await fetch(`https://serverowned.onrender.com/api/stats?mode=${mode}`, {
    ...
    async function fetchStats() {
        try {
            const data = await fetchWithRetry("https://serverowned.onrender.com/api/stats");
            setStats(data);
        } catch (err) {
    ...
    async function fetchConversationStats() {
        try {
            const data = await fetchWithRetry("https://serverowned.onrender.com/api/conversations");

            if (Array.isArray(data)) {
                const total = data.length;
                const avg =
                    total > 0
                        ? Math.round(data.reduce((sum, c) => sum + (c.history?.length || 0), 0) / total)
                        : 0;
    ...
    const viewClientConvos = async (clientOrId) => {
        try {
            // accept either client object or string clientId
            const clientId = typeof clientOrId === "string" ? clientOrId : clientOrId.clientId;

            // If caller passed full client object, set editingClient to it. Otherwise try to find it in stats.clients.
            if (typeof clientOrId === "object") {
                ...
                else setEditingClient({ clientId }); // fallback minimal object
            }
    ...
    const saveClient = async () => {
        try {
            ...
            console.log("Saving client with form:", form, "editingClient:", editingClient);

            // Use clientId (not Mongo _id) in the URL for update
            const url = editingClient && editingClient.clientId
                ? `https://serverowned.onrender.com/api/clients/${editingClient.clientId}`
                : "https://serverowned.onrender.com/api/clients";
            ...
            if (!res.ok) {
                // show any backend error in console
                console.error("❌ Save failed:", resJson);
                return;
            }

            // The POST route returns { client, user }, while PUT returns the client doc directly.
            const savedClient = resJson.client ? resJson.client : resJson;
            const savedClientId = savedClient.clientId; // this is what the server expects for uploads and routes

            // Upload any files to /upload/:clientId (server expects clientId)
            for (const f of files) {
                if (!f.file || !f.name) continue;
                const formData = new FormData();
                formData.append("file", f.file);
                formData.append("name", f.name);
                await fetch(`https://serverowned.onrender.com/upload/${savedClientId}`, {
                    method: "POST",
                    body: formData,
                    credentials: "include",
                });
    ...
    const deleteClient = async (clientId) => {
        try {
            ...
            // If the removed client was open in the edit form, reset the form
  ...
  try {
    await fetch(`https://serverowned.onrender.com/admin/renew/${clientId}`, {
      method: "POST",
      credentials: "include", // include admin token
    });

    await fetchStats(); // refresh table after renew
  } catch (err) {
...
async function renewAllClients() {
  try {
    await fetch("https://serverowned.onrender.com/admin/renew-all", {
      method: "POST",
      credentials: "include",
    });

    await fetchStats(); // refresh table after renew-all
  } catch (err) {
    ...
    const deleteFile = async (fileId) => {
        try {
            await fetch(`https://serverowned.onrender.com/upload/${fileId}`, {
                method: "DELETE",
                credentials: "include", // 🔑 sends the token cookie
            });
    ...
    const handleLogout = async () => {
        try {
            const res = await fetch("https://serverowned.onrender.com/api/logout", {
                method: "POST",
                credentials: "include", // ensure cookies are cleared server-side
            });
                ...
                <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 rounded-2xl shadow-sm">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="text-right">
                            ...
                            <div className="font-medium text-slate-800 text-sm">{localStorage.getItem("userEmail") || "Admin"}</div>
                        </div>
            ...
            <TableCell>{client.tourRequests?.toLocaleString() || 0}</TableCell>
                                            <TableCell className="flex gap-2">
                                                ...
                                                <Button
                                                 size="sm"
                                                    variant="secondary"
  onClick={() => renewClient(client.clientId)}
...
>
  Renew Month
                                                ...
                                                <Button
                                                    size="sm"
                                                    variant="secondary"
                                                    onClick={() => viewClientConvos(client.clientId)}
                                                >
                                                    View Convos
                                                </Button>
                                            ...
                                            <TableCell>
                                                <Button
                                                    ...
                                                    onClick={async () => {
                                                        try {
                                                            // Toggle active between true/false
...
<Dialog open={showForm} onOpenChange={setShowForm}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          <Input
            ...
            value={form.igId || ""}
            onChange={e => setForm({ ...form, igId: e.target.value })}
          />
          <Input
  ...
  className="
    ...
    border rounded
    p-2
    min-h-[120px]     /* bigger height */
    sm:min-h-[150px]  /* even bigger on larger screens */
    focus:outline-none focus:ring-2 focus:ring-sky-200
    resize-vertical   /* allow manual resizing if you want */
  "
/>
  ...
  <DialogContent className="w-full sm:max-w-3xl mx-auto max-h-[90vh] overflow-y-auto p-4 sm:p-6">
    <DialogHeader>
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-2 mt-3 mb-2">
        ...
        <Button
          variant={selectedSource === "web" ? "default" : "outline"}
